...
L = float(input("调节晶体厚度(0.0001<=L<=0.001):"))
c = 3e+8
...
alpha1 = float(input("请输入alpha1(角度): "))
beta1 = float(input("请输入beta1(角度): "))
print(alpha1,beta1)
...
a: str = input('请选择你的晶体（1=LiNbO_3，2=β-BaB_2O_4, 3=LiIO_3， 4=LiTaO_3）')
b: str = input('请选择三波混频类型（其中1=倍频，2=和频，3=差频）:')
d: str = input('请选择你的匹配类型(其中1=ooe,2=eoe，3=eeo,4=eoo):')


def cal(i, j, l, a, b, d, s, p):
            ...
            if b == "1":
                ...
                if d == "3":
                    ...
                    if 0 <= solution_1[0] <= sp.pi / 2 and 0 <= solution_1[1] <= sp.pi / 2 and abs(
                        ...
                        print("theta1", sp.deg(solution_1[0]))
                        print("phi1", sp.deg(solution_1[1]))
                        print('第一方程', solution_1[0] - (solution_2[0] + solution_2[1]) / 2)
                        print('第二方程',ne_omiga1_theta1 * omiga1 * sp.cos(solution_1[0]) - no_omiga2 * omiga2 * sp.cos(
                                  solution_2[0]) - no_omiga3 * omiga3 * sp.cos(solution_2[1]))
                        print("theta2", sp.deg(solution_2[0]))
                        print("theta3", sp.deg(solution_2[1]))
...
phi2 = float(input("设定phi2为:"))
phi3 = float(input("设定phi3为:"))
ang = forEach()
...
if ang[2] == "1":
    ...
    if ang[3] == "1":
        ...
        m3 = np.array([[sp.sin(ang[8]) * (-1) * sp.cos(theta2) * sp.cos(phi2)],
                       ...
                       [-sp.sin(ang[8]) * sp.cos(theta2) * sp.sin(phi2) + sp.cos(ang[8]) * sp.cos(theta2) * sp.cos(phi2)]])
        # n1 = ang[4]
        # n2 = ang[5]
        # omiga1 = ang[6]
        ...
        m3 = np.array([[sp.cos(ang[7]) * sp.cos(ang[8]) * sp.cos(theta2) * sp.cos(phi2)],
                       ...
                       [sp.sin(ang[7]) * (-1) * sp.cos(theta2) * sp.cos(phi2) + (-1) * sp.cos(ang[7]) * sp.cos(
                           ang[8]) * sp.sin(theta2)],
                       [sp.cos(ang[7]) * sp.cos(ang[8]) * sp.cos(theta2) * sp.sin(phi2) + sp.cos(ang[7]) * sp.sin(
...
def angle(a31, a32, a33, a41, a42, a43):
                ...
                for a41 in np.arange(0.00001, sp.pi / 2, 1):
                    for a42 in np.arange(0.00001, sp.pi / 2, 1):
                        for a43 in np.arange(-5, 5, 1):
                            res2 = angle(a31, a32, a33, a41, a42, a43)
                            if res2[0] == 1:
                                return
forAngle()
